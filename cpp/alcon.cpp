...
void CAlconDatabase::Evaluation( map<int, vector<string> > outputs ){
	...
	for( vector<int>::iterator it=ids.begin(); it!=ids.end(); it++ ){
		...
		// Groundtruth::正解文字を記録したデータのこと

		vector<string> o = outputs[*it]; // My output(vector)の要素取り出し
		vector<string> g = GetUnicodesForOneTarget( *it ); // Groundtruth(vector)の要素取り出し
